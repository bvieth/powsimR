...
The following formula will help you to calculate the number of molecules of spike-ins:

  \begin{displaymath}
    Y_j = c_j * V * 10^{-3} * {2}{x^2} * 10^{-18} * {Avogadro}, \quad j=1,\dots,92
  \end{displaymath}

The number of molecules Y_j for each ERCC spike-in species is equal to the molar concentration c_j (attomoles per microliter), the dilution factor D, the volume V in nanolit


<<echo=TRUE,eval=FALSE>>=
# download count table of spike-ins
githubURL <- "https://github.com/bvieth/powsimRData/raw/master/data-raw/kolodziejczk_cnts.rda"
download.file(url = githubURL, destfile= "kolodziejczk_cnts.rda", method = "wget")
load('kolodziejczk_cnts.rda')
# download spike-in information table
