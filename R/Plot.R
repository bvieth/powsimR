...
#' @author Beate Vieth
#' @importFrom ggplot2 ggplot geom_bar geom_line theme geom_hline xlab ylab coord_flip scale_y_continuous geom_point scale_fill_gradientn stat_density2d labs theme_minimal theme_classic element_text ylim
...
plotParam <- function(estParamRes, annot=TRUE) {
  ...
  if(estParamRes$RNAseq=="bulk" | estParamRes$estS<15) {
    ...
    meanvsdisp.plot <- ggplot2::ggplot(data=meanvsdisp.dat, aes(x=Means, y=Dispersion)) +
      ...
      ggplot2::geom_line(aes(x=estParamRes$meandispfit$x, y=estParamRes$meandispfit$y),
                         linetype=1, size=1.5, colour="orange") +
      ggplot2::geom_line(aes(x=estParamRes$meandispfit$x, y=estParamRes$meandispfit$upper),
                         linetype=2, size=1, colour="orange") +
      ggplot2::geom_line(aes(x=estParamRes$meandispfit$x, y=estParamRes$meandispfit$lower),
                         linetype=2, size=1, colour="orange") +
...
plotEvalDE <- function(evalRes, rate=c('marginal', 'stratified'), quick=TRUE, annot=TRUE) {
  ...
  if(rate=='marginal') {
    if(quick) {
      ...
      # marginal in one
      grandplot <- ggplot2::ggplot(data = dat.marginal.long, aes(x=Var1, y=value, fill=L1)) +
      ...
      # faceted marginal
      facetplot <-  ggplot2::ggplot(data = dat.marginal.calc, aes(x=Var1, y=Expectation, fill=L1, color=L1)) +
        ggplot2::geom_line(aes(group=L1)) +
        ...
        ggplot2::geom_hline(data = refval, aes(yintercept = ref), linetype="dashed", color='grey')
      # annotation under plot
    ...
    if(!quick) {
      ...
      limits <- ggplot2::aes(ymax = Expectation + Deviation, ymin= Expectation - Deviation)
      # marginal in one
      grandplot <- ggplot(data = dat.marginal.long, aes(x=Var1, y=value, fill=L1)) +
        ggplot2::geom_boxplot() +
        ggthemes::theme_base() +
        ggplot2::xlab(NULL) +
        ggplot2::ylab('Rate') +
  ...
  if(rate=='stratified') {
    if(quick){
      ...
      refval <- data.frame(L1 = c("FDR", "TPR"), ref = c(evalRes$alpha.nominal, 0.8))
      facetplot <-  ggplot2::ggplot(data = dat.stratified.calc, aes(x=Var1, y=Expectation, fill=Var2, color=Var2)) +
        ggplot2::geom_point() +
        ggplot2::geom_line(aes(group=Var2)) +
        ggplot2::geom_pointrange(limits) +
        ggthemes::theme_base() +
        ggplot2::xlab(NULL) +
        ggplot2::ylab('Rate') +
      ...
      strataplot <- ggplot2::ggplot(data = dat.genes.calc, aes(x=Var1, y=Expectation, fill=L1)) +
