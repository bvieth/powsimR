...
With the following command, we estimate and plot the parameters for the embryonic stem cells using the different media as batch annotation [@Kolodziejczyk2015-vn] (Figure \@ref(fig:paramsplot)). As expected for single cell RNA-seq, the variability (i.e. dispersion) and dropout rates are high. Furthermore, the dispersion strongly depends on the mean and does not level off with higher mean values compared to bulk data.
...
kolodziejczk_grps <- data.frame(Culture = c(rep("alternative", 194),
                                ...
                                stringsAsFactors = F)
dim(kolodziejczk_cnts)
dim(kolodziejczk_grps)
...
```{r paramsplot, echo=T, eval=T, include=T, fig.height = 15, fig.width=20, fig.cap="Estimated parameters for Kolodziejczyk data set."}
plotParam(TwoiLIF.params, annot = T)
```
