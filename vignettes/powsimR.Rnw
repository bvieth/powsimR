...
biocpackages <- c("S4Vectors", "DEDS",   "AnnotationDbi", "Biobase",
                  ...
                  "DESeq2", "baySeq", "NOISeq", "EBSeq", "MAST", "scde",
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{NBparams.pdf}
...
The following formula can be used for calculating the number of molecules of spike-ins:
...
% The molecule calculation is implemented in \Rfunction{calculateSpike}.
...
\begin{description}
  ...
  \item[DEmethod] The differential testing method. The user can choose between 11 methods in total.  7 developed for bulk, 4 developed for single cells (see the detail section of \Rfunction{simulateDE}).
...
% \begin{figure}[h]
% \centering
% \includegraphics[width=0.8\linewidth]{margpower.pdf}
% \caption{Log2 fold change examples for gamma, uniform and normal distribution}
% \label{fig:margpower}
% \end{figure}
%
% \begin{figure}[h]
% \centering
% \includegraphics[width=0.8\linewidth]{condpower.pdf}
% \caption{Log2 fold change examples for gamma, uniform and normal distribution}
% \label{fig:condpower}
% \end{figure}
