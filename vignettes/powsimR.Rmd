...
```{r schematic, fig.cap="PowsimR schematic overview. (A) Estimation (B) Simulation (C) Evaluation.", echo=F, eval=T, include=T}
...
To predict the dispersion given a random draw of mean expression value observed, we apply a locally weighted polynomial regression fit (orange line in Figure \@ref(fig:geneparamsplot) D). To capture the variability of dispersion estimates observed, a local variability prediction band is applied.
...
The following command sets up simulations with 10,000 genes, 5\% genes being DE, log fold change sample from a narrow gamma distribution and parameter estimates based on Smartseq2 libraries in Ziegenhain dataset. 
